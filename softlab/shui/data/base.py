...
class DataGroup():
    ...
    def meta(self) -> Dict[str, Any]:
        """All metadata of group"""
        return self._meta

    @meta.setter
    def meta(self, m: Union[Dict[str, Any], Sequence[Tuple[str, Any]]]) -> None:
        if isinstance(m, Dict):
            m = m.items()
        self._meta = dict(filter(
            lambda pair: isinstance(pair, Tuple) and len(pair) == 2 and \
                isinstance(pair[0], str) and len(pair[0]) > 0,
            m))

    def get_meta(self, key: str) -> Any:
        """Get single metadata with given ``key``"""
        return self._meta.get(key, None)

    def set_meta(self, key: str, value: Any) -> None:
        """
        Set metadata with ``key`` and ``value``
    ...
    def pop_meta(self, key: str, def_value: Any = None) -> Any:
        """Delete metadata with given ``key`` and return its previous value"""
        key = str(key)
        if len(key) == 0:
            raise ValueError('Key of metadata must be non-empty')
        return self._meta.pop(key, def_value)
